...
const ENTRIES = [
  {
    eyebrow: 'HALL OF SPONSORS',
    title: '星际酒馆赞助榜',
    desc: '感谢每一位帮助社区持续运营、举办活动并支持奖励计划的酒友。',
    ctaLabel: '查看完整赞助榜',
    href: '/sponsors',
  },
  {
    ...
    title: '星际酒馆名人堂',
    desc: '记录赛事冠军、竞技荣誉与属于酒馆社区的历史时刻。',
    ctaLabel: '进入名人堂',
    href: '/hall-of-fame',
  },
] as const

function SupportEntry({ entry }: { entry: (typeof ENTRIES)[number] }) {
  return (
...
export async function CommunitySupport() {
  ...
  const HONORS = [
    { value: formatSponsorTotal(total), label: '累计赞助', en: 'TOTAL SUPPORT' },
    { value: String(sponsorCount), label: '社区赞助者', en: 'SPONSORS' },
    { value: '1000+', label: '社区成员', en: 'MEMBERS' },
  ] as const
    ...
    >
      <div className="site-container">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)] lg:items-end lg:gap-16">
          ...
          >
            {HONORS.map((honor) => (
              <div key={honor.en} className="flex flex-col gap-2">
...
/>
        ...
        <div className="mt-14 grid gap-10 border-t border-border pt-12 sm:grid-cols-2 sm:gap-0 sm:divide-x sm:divide-border lg:mt-20 lg:pt-14">
          <div className="sm:pr-10 lg:pr-14">
            <SupportEntry entry={ENTRIES[0]} />
          </div>
          <div className="sm:pl-10 lg:pl-14">
            <SupportEntry entry={ENTRIES[1]} />
          </div>
        </div>
      </div>
    </section>
