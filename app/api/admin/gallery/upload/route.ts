...
import { MAX_UPLOAD_BYTES, formatFileSize } from '@/lib/upload-limits'
...
export async function POST(request: NextRequest) {
  ...
  if (file.size > MAX_UPLOAD_BYTES) {
    return NextResponse.json(
      {
        error: `图片文件过大：原始大小 ${formatFileSize(file.size)}，超过服务器支持的最大上传大小 ${formatFileSize(MAX_UPLOAD_BYTES)}。请压缩图片或裁剪尺寸后重新上传。`,
  ...
  const uniqueName = generateStorageFilename(file.name, file.type)
  ...
  if (uploadError) {
    console.error('[v0] Gallery upload error:', uploadError)
